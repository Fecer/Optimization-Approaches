...
//入泊+离开总时间短的先入港，优先吊所需时间少的货物，优先运所需时间少的货物
struct ship;
struct cargo;
int cargoN, shipN, warehouseN;                    //输入基本数量信息
...
struct ship                                         //货船
{
    ...
    friend bool operator < (ship a, ship b)         //总用时少的优先级高
    {
        return (a.timeIn + a.timeOut) > (b.timeIn + b.timeOut);
    }
};
        ...
        for(int j = 1; j <= deviceN; j++)
        {
            ...
            if(D[j].busy == 0)                                  //空闲塔吊
            {
                ...
                for(int i = 1; i <= berthN; i++)                //遍历泊位,起吊
                {
                    ...
                    {
                        if(min > qc[B[i].shipOrd].top().time)   //找到最小时间的货物
                        {
                            //记录下最好的货物
                            min = qc[B[i].shipOrd].top().time;
                            curBestB = i;
